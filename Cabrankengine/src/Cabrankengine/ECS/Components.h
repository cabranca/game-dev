...
namespace cbk::ecs {
	...
	struct COBBCollider {
		...
		COBBCollider(const VecType& halfExtents,
			const typename math::VecTraits<VecType>::OrientationType& orientation)
	...
	struct CPlaneCollider {
		...
		CPlaneCollider(const VecType& normal, float distance)
	...
	struct CCylinderCollider {
		...
		CCylinderCollider(float halfHeight, float radius)
