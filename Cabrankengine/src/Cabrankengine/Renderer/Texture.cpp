...
namespace cabrankengine::rendering {
	...
	Ref<Texture2D> Texture2D::create(const std::array<std::string, 6>& cubeMapPath) {
		switch (Renderer::getAPI()) {
		case RendererAPI::API::None:    CE_CORE_ASSERT(false, "RendererAPI::None is currently not supported!"); return nullptr;
		case RendererAPI::API::OpenGL:  return  createRef<platform::opengl::OpenGLTexture2D>(cubeMapPath);
		}
