...
class ExampleLayer : public Layer {
  public:
	ExampleLayer() : Layer("Example"), m_CameraController(PerspectiveCamera(PI / 4.f, 16.f / 9.f, 0.1f, 100.f)) {
		...
		auto shader = m_ShaderLibrary.load("assets/shaders/EnvMap.glsl");
		...
		// setupCube(lightningCubeVertices, 36 * 8);
		
		auto skyboxShader = m_ShaderLibrary.load("assets/shaders/Skybox.glsl");
		m_SkyboxTex = Texture2D::create(m_SkyboxPaths);
		m_SkyboxTex->bind(0);
	...
	void onUpdate(Timestep delta) override {
		...
		Renderer::submit(skyboxShader, m_CubeVA, scaleUniform(10.f), true);

		auto envMapShader = m_ShaderLibrary.get("EnvMap");
		envMapShader->setFloat3("u_CameraPos", camera.getWorldPosition());
		Renderer::submit(envMapShader, m_CubeVA, scaleUniform(1.f), false);
  ...
  private:
	...
	void setupCube(float vertices[], size_t count) {
		m_CubeVA = VertexArray::create();

		Ref<VertexBuffer> squareVB = VertexBuffer::create(vertices, sizeof(float) * count);

		squareVB->setLayout({ { ShaderDataType::Float3, "pos" }, { ShaderDataType::Float3, "normal" }, { ShaderDataType::Float2, "texCoord" } });

		m_CubeVA->addVertexBuffer(squareVB);

		uint32_t lightningCubeIndices[count];
